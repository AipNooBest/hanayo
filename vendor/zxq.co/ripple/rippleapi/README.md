...
This is the source code for Debiki's API.

https://github.com/AipNooBest/api
...
* In the API, to get a specific item, you need to do e.g. `/users?id=1009`. It's much more useful to have these in the URL path directly (`/users/1009`) for a number of reasons:
  ...
  * There is no multiple ways of addressing a specific user. There is a single way: IDs. In the Debiki API, you can specify an username instead of an ID to get a specific user, but this is prone to failure in the event of the user changing the username, whereas an ID cannot (should not) change.
